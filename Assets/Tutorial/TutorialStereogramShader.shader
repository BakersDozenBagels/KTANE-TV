	...
	SubShader {
		...
		Pass
		{
			...
			float random (float2 st) {
				//return st.x;
				float OUT = sin(dot(floor(abs(st.xy * 300.0)),float2(12.9898,78.233)) + time * 100)*43758.5453123;
			...
			frag2screen main_frag(vert2frag IN)
			{
				...
				float2 uv = floor(IN.texcoord.xy * 300) / 300; 								// transform texture coordinate into depth map space 
						float2 uv2 = uv;													// (removing first strip) and get depth value 
				...
				float4 tex3;
				 ...
				 // previous strip translated by the displace factor
					...
					float a2 = (uv2.x/(strips_info.y - 1.0)) * strips_info.x;
						float displace2 = a2 * tex.x * abs(depth_factor) * strips_info.y;
						uv2.x = floor((uv2.x - strips_info.y + displace2) * 300) / 300; 
						tex3 = tex2D(_MainTex, uv2);
				...
				OUT.color = random(uv) * pow(tex2, fade_factor) * pow(tex3, fade_factor_two); 
				return OUT;
			} 
			ENDCG
		}
